#!/bin/bash
# Skill: Disable TTS for tool
# Description: Disable text-to-speech for a specific CLI tool
...
if [ $# -eq 0 ]; then
    ...
    echo "Usage: skill-disable <tool>" >&2
    echo "Available tools: claude-code, opencode, codex-cli, gemini-cli" >&2
    exit 1
fi

# Execute CLI disable command
exec "$NODE_CMD" "$CLI_PATH" "disable" "$1"